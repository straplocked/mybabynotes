...
| `get-household-status` | `profile:read` | — | Members and roles, who's covering (null when nobody is), the children, the latest cover, every running timer with elapsed time |
...
| `start-timer` | `timer:write` | `type` (`nurse`/`pump`/`sleep`/`tummy`), `baby_id?`, `minutes_ago?`, `started_at?` | Starts a timer; timers stack, and re-starting an identical session (same type, child, starter) returns the one already running. `minutes_ago` (or an epoch-ms `started_at`) backdates a timer started after the baby already went down — clamped to now and at most a day back |
