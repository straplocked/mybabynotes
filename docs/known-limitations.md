...
- **A nap's row time and its "Slept" card disagree, on purpose.** Every entry reads at the moment it *started* — sleep and tummy time stamp the wake-up on the wire, and the UI subtracts the duration to place them — but the "Slept" since-card, the "Last nap ended" handoff row and the wake-window insight deliberately keep measuring from the **end**, because the open wake window is the number you act on. So a nap can sit at 3:00 AM in the timeline while its card reads "1h ago · 4:30 AM". Integrations see the raw wake-up stamp ([integrations.md](integrations.md#entry-semantics)).
...
- **A handoff ask can't be withdrawn.** Once you've asked, the only ways out are someone accepting it or you handing back (which cancels any pending ask). Until then the Now screen reads "Waiting for {name}" and offers a re-ask, which refreshes the request and re-pings — there's no `/shifts/cancel`.
