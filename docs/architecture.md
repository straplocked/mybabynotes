...
- **Now screen composition**: a header (baby + age, then a `swap_horiz` shift button and the duty avatar), any running-timer cards, an incoming handoff ask, the since-card grid, and today's timeline. The header's two icon buttons split what used to be one "X · on duty" pill: the shift button opens the shift sheet (accent-filled with a pulsing dot only when an ask is waiting on you), the avatar carries whose duty it is in its colour/initial and taps through to Settings. **Every other shift surface lives in that sheet** — your live checklist, the off-duty read-only view of theirs, and the "Start your shift" framing — so Now stays a log. The one exception is an incoming ask, which stays an inline card because it needs answering, not browsing.
