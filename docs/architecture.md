...
- **The log sheet's stamp is a ladder, and the day is its top rung.** The common case never touches a date: the sheet opens stamped *now*, the `−5/−15/−1h` nudges cover the near past, and the time picker's midnight rule (a time later than now means last night) reaches back through the small hours. Past that it used to stop — a 3am backfill of a feed from two nights ago was unloggable — so the day is its own control behind an **Advanced** link, right-justified on the Cancel row. Opening it reveals a Today/Yesterday/calendar chip row plus a plain reading of what will be saved (`Sat, Sep 5 · 11:40 PM`, and `→ 12:25 AM` for a span, whose end the sheet otherwise never prints). It's folded away because the day is almost never the answer, and folded *open* on an edit whose entry isn't from today — there it's the field you came for. How each opening *starts* is a device pref (`advancedDefault`, Settings → Appearance, persisted in `PERSIST` alongside `timerSpot`), **off by default**: the drawer costs one tap on the nights it matters, which is the right trade for the parent who logs "now" and the wrong one for the parent who backfills, so each phone answers for itself. It never syncs — the sheet's shape is nobody else's business. The timer path has no Advanced link at all: a live timer starts now. `setDay()` moves the calendar date under the time already on the sheet; today plus a time that hasn't come round yet clamps to the moment itself (for a span, to a session that ends now), since nothing can be logged in the future. **Which day a picked time lands on** comes from *today* — a time later than now means last night — unless the day was chosen on purpose (`dayPicked`, set by the day control, or an edit, where the entry's own day is the point). It deliberately does *not* read the day the sheet is showing: a 45m nap opened at 12:20am already shows 11:35 PM yesterday, and resolving 12:05 AM against that day filed the nap a full day early. **What the sheet holds still while the duration changes** (`anchorsStart()`): a fresh sheet anchors on now as the session's *end* — "just woke up, slept 45m", so a longer duration reaches further back — but once a time is picked or an entry is being edited, the *start* the sheet shows stays put and the wake-up moves, so the 9:00 PM a parent just typed never silently becomes 3:30 PM.
...
- The bearer token lives in `localStorage`, so script injection is the threat that matters: nginx enforces a `Content-Security-Policy` (`default-src 'self'`; `script-src 'self'` plus the sha256 of the one inline script in `index.html`; `style-src 'unsafe-inline'` for the comp's inline styles; Google Fonts as the only third-party origins; `object-src 'none'`, `base-uri`/`form-action`/`frame-ancestors 'self'`). The header is repeated in the `/assets/` and `/sw.js` locations because a location-level `add_header` replaces the server-level set. `src/test/csp.test.js` recomputes the inline script's hash and pins the policy in both nginx files.
- The password-reset mail links to `APP_URL/#reset=<token>&email=…`: the token rides the URL fragment, which the browser never sends, so it stays out of nginx/proxy access logs and Referer headers. The SPA reads it on boot (the legacy `?reset=` form is still honoured for mails already delivered, fragment wins), keeps it in memory only (never in the `PERSIST` cache), and `replaceState`s it out of the address at mount. `BabylogApiTest::test_the_reset_link_carries_its_token_in_the_fragment_not_the_query` and the "arriving from a password-reset email" block in `src/test/app.test.jsx` pin both halves.
