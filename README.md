...
<p align="center">
  <picture><source media="(prefers-color-scheme: dark)" srcset="docs/media/now-dark.png"><img src="docs/media/now.png" alt="Now screen — a running nursing timer, since-cards (the Fed card flipped to “Feeding now”), today's timeline, and the header's cover and settings buttons" width="23%"></picture>
  <picture><source media="(prefers-color-scheme: dark)" srcset="docs/media/log-dark.png"><img src="docs/media/log.png" alt="The log sheet — every entry type one tap away, with the timer framing (its start backdatable with −5/−15/−1h or a picked time) and a “Log a past feed” escape hatch" width="23%"></picture>
  <picture><source media="(prefers-color-scheme: dark)" srcset="docs/media/history-dark.png"><img src="docs/media/history.png" alt="History screen — 7-day stats tiles and the per-day charts this household picked, feeds and sleep-in-hours" width="23%"></picture>
...
- **Covers, not a rota.** Most of the time nobody is "on duty" — you're both just there, and the app says so. A **cover** is what you switch on when that stops being true: one of you takes the night, or a grandparent sits for an afternoon. Nobody covering is the resting state a new household starts in and the state every cover returns to, so the app never insists somebody is in charge when nobody needs to be. A parent can **assign** a cover outright — "Gran is covering until 4" — because by the time you've reached for the phone she's already holding the baby; or you can ask, and that waits for a yes. Either way the person setting it up *authors* it: a plan drafted from the baby's rhythm whose items you can retime, drop, or add to (feed, nursing, pump, sleep, tummy time, bath, meds), an "until" time, and a note. It lands on the other phone as an actual plan rather than a sentence. Logged feeds tick it off live, everyone else watches progress read-only without asking, and a cover ends three ways: you end it, the "until" ends it on its own, or you hand it to a named person — each producing a summary instead of a "when did you…" conversation. Unfinished items — a dose that never happened — carry into the next cover's plan rather than evaporating. The whole surface sits one tap behind a header button, so Now stays a log; only an incoming ask interrupts it, because that one needs answering.

<p align="center">
  <img src="docs/media/duty.png" alt="The cover sheet with nobody covering — where things stand right now, the drafted plan, an until time, and a start button" width="30%">
  <img src="docs/media/handoff.png" alt="Composing a cover — who is taking it, whether they are here now or need asking, the plan with each item retimeable, an until time, and a note" width="30%">
  <img src="docs/media/incoming.png" alt="The same cover ask on the other phone — your note, the plan you wrote, and one tap to take over" width="30%">
</p>

- **Live timers, shared across phones.** Nursing, pumping, sleep, and tummy time run as server-backed start/stop timers visible on both devices; stopping a nursing, sleep, or tummy time timer auto-logs the entry, and stopping a pump timer opens the log sheet with the duration pre-filled so you can add the amount. **Anyone in the household can stop any timer** — a nap outlives the cover change that happens mid-nap — and the entry stays credited to whoever ran the session, not whoever pressed Stop. Started the timer late? Backdate it from the sheet before you hit Start — tap −5/−15/−1h, drag a chip up to walk the start further back ("Started 25m ago"), or pick a time.
...
- **Now & History.** Since-cards the household picks from seven (fed, pumped, diaper, slept, tummy time, bath, meds) — each flips to the live session while a timer of its kind is running, so “Slept · 4h ago” never sits on screen through a nap — today's totals, an editable timeline with one-shot undo (add, edit, or delete) where every entry — a nap included — reads at the time it *started*, 7-day stats, and **the bar charts the household picks** from that same seven, so a screen that only ever showed feeds and diapers can show naps instead. Sleep is the one chart counted in hours a day rather than times a day, since three naps could be forty-five minutes or six hours. Every bar taps through to that day, the header tally counts whatever you picked rather than always feeds and diapers, and a live age header runs off the baby's birth date.
- **What the log notices.** Four calm insight cards sit under the charts, and each of them says nothing at all rather than something wrong. The **feeds rhythm** — roughly how long between feeds this week, against what's typical at this age. That **rhythm changing**: “Feeds have tightened to about every 2h” when the last day runs 30% closer than the settled week behind it, naming cluster feeding as the likely and normal reason, then “Back to about every 4h between feeds” once the tighter spell has passed — read from medians, not averages, because an average is exactly what hides a change (4h that became 2h reads as “roughly every 3h”), with cluster bursts folded into one beat and a steady week left in silence. The **wake window** against the age-typical band. And **the nap the days keep agreeing on** — “Naps around 12:05 PM most days · on 5 of the last 7 days they went down between 11:58 AM and 12:20 PM, for about 2h” — clustered on when each nap *began*, needing three separate days inside ± 45 minutes before it claims a pattern, and leaving overnight sleep out of it.
- **Notifications without a cloud.** Self-hosted Web Push (VAPID keys generate themselves — no FCM/APNs account): cover asks and endings, a partner starting a timer, opt-in partner activity, feed-gap and wake-window reminders, a daily meds nudge, and quiet hours — all per-parent.
...
- **Invite-only by design.** The first account claims the instance and invites the rest of the household (emailed code with SMTP, shareable on-screen code without) — each invite as a **parent** (full control) or a **caregiver** (logs, timers, and covers, but can't touch settings or membership). Password reset works the same way. Up to six adults and ten children per household — enough for a doula and twins, still an appliance, not a platform.
...
| Covers & handoffs | First-class: nobody on duty by default, assign a sitter, plan, auto-ending window, summary | — |
...
React 18 + Vite PWA · Laravel 13 API (SQLite by default, Postgres supported; Sanctum) · Laravel Reverb websockets · nginx · Docker. Invite-only registration, throttled auth, client-generated entry ids with tombstone-synced deletes, poke-to-pull realtime (the server broadcasts "something changed", never data), computed cover reports, expiring tokens. Details: [docs/architecture.md](docs/architecture.md).
...
| Doc | What's in it |
|---|---|
| [docs/architecture.md](docs/architecture.md) | System design, sync model, realtime, notifications, cover semantics, key decisions |
