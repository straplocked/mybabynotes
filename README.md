...
- **Resume a sleep that wasn't over.** A baby who stirs for five minutes had one nap with a gap in it, not two naps, so the newest sleep carries a **Resume** button on its row: the timer picks back up from where that nap began, and stopping it rewrites that same entry — one longer session, not a second one stacked on top. It's offered only while that sleep is still the newest thing logged; once anything lands after it, the session is closed for good.
