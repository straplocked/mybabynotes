...
- **The background art carries no field of its own.** `public/art/*.png` (mirrored in `design/art/`) is motifs on transparency, so the household background tint shows through — never re-flatten them onto cream. The stack is art → accent glows → `.bg-wash` (the comp's legibility gradient, painted in the backdrop's own colour). In dark that order inverts via `html.dark .bg-wash { z-index: -1 }`: a same-colour wash over dimmed art is what turned the whole screen grey, so the motifs sit *on top* of the gradient at 0.6 opacity instead. Keep both wash layers (app + entry sheet) tagged `.bg-wash`.
- **Dark depth comes from the marketing site, not from this repo.** `mybabynotes.app` (`~/dev/mybabynotes-site`, `css/site.css`) is the accurate reference for dark mode: `--bg:#161019`, `--frame:#0A070C`, motifs at full colour over it via `<parallax-bg opacity="0.6">`. `THEME_BGS_DARK` in [src/App.jsx](src/App.jsx) matches that depth — oklch ≈0.185 L / ≈0.020 C — with each household background keeping its own hue (cream *is* the site value verbatim). Don't lift these back toward the old ≈0.23 L ladder; that's what read as grey. The dark `theme-color` in [index.html](index.html) and `color_scheme_dark` in the manifest must move with `--bg`.
